...
class Strategy(metaclass=ABCMeta):
    ...
    def buy(self, *,
            ...
            tag: object = None,
            spread: Optional[float] = None) -> 'Order':
        """
        ...
        `spread` parameter allows you to override the default spread for this specific order.
        """
        assert 0 < size < 1 or round(size) == size >= 1, \
            "size must be a positive fraction of equity, or a positive whole number of units"
        return self._broker.new_order(size, limit, stop, sl, tp, tag, spread=spread)
...
class Position:
    ...
    def close(self, portion: float = 1., spread: Optional[float] = None):
        """
...
class Order:
    ...
    def spread(self):
        """
        Custom spread for this order, or None to use the default spread.
        """
...
class Trade:
    ...
    def close(self, portion: float = 1., spread: Optional[float] = None):
        ...
        order = Order(self.__broker, size, parent_trade=self, tag=self.__tag, spread=spread)
...
class _Broker:
    ...
    def new_order(self,
        ...
        is_long = size > 0
        assert size != 0, size
        adjusted_price = self._adjusted_price(size, spread=spread)
        ...
        order = Order(self, size, limit, stop, sl, tp, trade, tag, spread)
    ...
    def _adjusted_price(self, size=None, price=None, spread=None) -> float:
        """
        Long/short `price`, adjusted for spread.
        In long positions, the adjusted price is a fraction higher, and vice versa.
        If `spread` is provided, it overrides the default spread for this calculation.
        """
        effective_spread = spread if spread is not None else self._spread
        return (price or self.last_price) * (1 + copysign(effective_spread, size))
    ...
    def _process_orders(self):
        ...
        for order in list(self.orders):  # type: Order
            ...
            # In long positions, the adjusted price is a fraction higher, and vice versa.
            adjusted_price = self._adjusted_price(order.size, price, order.spread)
